...
class GTrans(MessagePassing):
    ...
    def forward(self, x, edge_index, edge_type,edge_vector):
        ...
        residual = x
        x = self.layer_norm(x)        
        return self.propagate(edge_index, x=x, edge_type=edge_type, edge_vector = edge_vector,residual=residual,num_nodes = num_nodes)

    def message(self, x_j, x_i, edge_index_i, edge_type,edge_vector,num_nodes):
        '''
        ...
        for i in range(self.n_heads):
            ...
            attention_norm = softmax(attention, edge_index_i,num_nodes)  # [num_edge,num_edge]
